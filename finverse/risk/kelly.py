...
class KellyResult:
    ...
    def summary(self):
        ...
        if self.full_kelly > 1.0:
            console.print(
                f"\n  [yellow]⚠  Full Kelly = {self.full_kelly:.0%} implies leverage. "
                f"This assumes Gaussian returns and stationarity — both false in practice.\n"
                f"     Fat tails (see EVT module) make this dangerously optimistic.\n"
                f"     Consider [bold]quarter Kelly ({self.quarter_kelly:.0%})[/bold] as a practical ceiling.[/yellow]"
            )
        elif self.full_kelly > 0.5:
            console.print(
                f"\n  [yellow]⚠  Full Kelly > 50% — consider half or quarter Kelly in practice.[/yellow]\n"
                f"  [dim]Note: Kelly assumes Gaussian returns. EVT analysis may suggest a lower fraction.[/dim]"
            )
...
def from_distribution(
    ...
    # Use log-returns for Kelly — more theoretically correct than simple returns
    # Simple returns can give Kelly > 1 trivially on high-Sharpe assets
    log_returns = np.log1p(returns)
    mu = float(np.mean(log_returns))
    sigma2 = float(np.var(log_returns))

    raw_kelly = mu / sigma2 if sigma2 > 0 else 0.0
    # Raw Kelly can legitimately exceed 1.0 — it means "use leverage"
    # Cap at 2.0 max (200%) for display; warn when > 50%
    full_kelly = float(np.clip(raw_kelly, -2.0, 2.0))
...
def multi_asset(
    ...
    return result    breakeven_kelly: float          # fraction where growth = 0
